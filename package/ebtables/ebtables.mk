...
EBTABLES_VERSION_FILE = Makefile
EBTABLES_VERSION_PATTERN = "^PROGVERSION_:=(.+)"
...
define EBTABLES_BUILD_CMDS
	$(MAKE) $(TARGET_CONFIGURE_OPTS) LIBDIR=/usr/lib/ebtables $(EBTABLES_STATIC) \
...
define EBTABLES_INSTALL_TARGET_CMDS
	for so in $(@D)/$(EBTABLES_SUBDIR)/*.so \
		...
		do \
		$(INSTALL) -m 0755 -D $${so} \
			$(TARGET_DIR)/usr/lib/ebtables/`basename $${so}` || exit 1; \
	done
