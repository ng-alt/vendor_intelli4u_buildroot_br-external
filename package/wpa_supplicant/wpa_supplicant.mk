...
define WPA_SUPPLICANT_CONFIGURE_CMDS
	if [ -e $(BR2_EXTERNAL_NETGEAR_PATH)/board/common/wpa_supplicant.defconfig ] ; then \
		cp $(BR2_EXTERNAL_NETGEAR_PATH)/board/common/wpa_supplicant.defconfig $(WPA_SUPPLICANT_CONFIG) ; \
	fi
...
define WPA_SUPPLICANT_BUILD_CMDS
	...
	$(TARGET_MAKE_ENV) CFLAGS="$(WPA_SUPPLICANT_CFLAGS)" \
		...
		LIBS="$(WPA_SUPPLICANT_LIBS)" LIBS_c="$(WPA_SUPPLICANT_LIBS)" \
		LIBS_p="$(WPA_SUPPLICANT_LIBS)" \
		$(MAKE) CC="$(TARGET_CC)" -C $(@D)/src/eap_peer-0.7.3
endef
...
define WPA_SUPPLICANT_INSTALL_EAP_PEER
	$(INSTALL) -m 0755 -D $(@D)/src/eap_peer-0.7.3/libeap.so.0.0.0 $(TARGET_DIR)/usr/lib/libeap.so.0.0.0
	ln -sf libeap.so.0.0.0 $(TARGET_DIR)/usr/lib/libeap.so.0
	ln -sf libeap.so.0.0.0 $(TARGET_DIR)/usr/lib/libeap.so
endef
...
define WPA_SUPPLICANT_INSTALL_TARGET_CMDS
	...
	$(INSTALL) -m 644 -D $(BR2_EXTERNAL_NETGEAR_PATH)/package/wpa_supplicant/wpa_supplicant.conf \
		$(TARGET_DIR)/etc/wpa_supplicant.conf
	$(WPA_SUPPLICANT_INSTALL_CLI)
	$(WPA_SUPPLICANT_INSTALL_PASSPHRASE)
	$(WPA_SUPPLICANT_INSTALL_DBUS)
	$(WPA_SUPPLICANT_INSTALL_WPA_CLIENT_SO)
	$(WPA_SUPPLICANT_INSTALL_EAP_PEER)
endef
