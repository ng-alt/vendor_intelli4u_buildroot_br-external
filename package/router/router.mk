...
ROUTER_PREPARED_MAKE_OPTS = SRCBASE=$(SRC_BASE_DIR) SRCBASE_A=$(BR2_TOPDIR)$(SRCBASE_A) \
...
ROUTER_DEPENDENCIES += $(if $(BR2_PACKAGE_NETTLE), nettle)
ROUTER_DEPENDENCIES += $(if $(BR2_PACKAGE_OPENSSL), openssl)
ROUTER_DEPENDENCIES += $(if $(BR2_PACKAGE_PCRE), pcre)
ROUTER_DEPENDENCIES += $(if $(BR2_PACKAGE_PORTMAP), portmap)
ROUTER_DEPENDENCIES += $(if $(BR2_PACKAGE_SDPARM), sdparm)
ROUTER_DEPENDENCIES += $(if $(BR2_PACKAGE_SQLITE), sqlite)
ROUTER_DEPENDENCIES += $(if $(BR2_PACKAGE_USB_MODESWITCH), usb_modeswitch)
ROUTER_DEPENDENCIES += $(if $(BR2_PACKAGE_WPA_SUPPLICANT), wpa_supplicant)
